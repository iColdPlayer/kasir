...
class BaseCSVField(forms.Field):
    ...
    def __init__(self, *args, **kwargs):
        widget = kwargs.get('widgets') or self.widget
        kwargs['widgets'] = self._get_widget_class(widget)
    ...
    def _get_widget_class(self, widget):
        ...
        # complain since we are unable to reconstruct widgets instances
        assert isinstance(widget, type), \
            "'%s.widgets' must be a widgets class, not %s." \
